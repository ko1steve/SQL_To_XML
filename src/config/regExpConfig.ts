...
export class RegExpConfig {
  public static readonly ALL_MSSQL_DDL_VALID_REGEXP: RegExp = /^(?:(?:CREATE\s+(?:TABLE|VIEW|(?:UNIQUE\s+)?(?:CLUSTERED\s+(?:COLUMNSTORE\s+)?|NONCLUSTERED\s+)?INDEX\s+\S+\s+ON|PROCEDURE|PROC|FUNCTION|TRIGGER|SYNONYM|SEQUENCE|SCHEMA|PARTITION\s+(?:FUNCTION|SCHEME))\s+(?!(?:@|#|##)))|(?:ALTER\s+(?:TABLE|VIEW|INDEX\s+\S+\s+ON|PROCEDURE|PROC|FUNCTION|TRIGGER|SEQUENCE|SCHEMA|PARTITION\s+(?:FUNCTION|SCHEME))\s+(?!(?:@|#|##)))|CREATE\s+OR\s+ALTER\s+(?:PROCEDURE|PROC|FUNCTION|TRIGGER)\s+(?!(?:@|#|##))|DROP\s+(?:PROCEDURE|PROC|FUNCTION|TRIGGER|TABLE|VIEW|INDEX|SYNONYM|SEQUENCE|SCHEMA|PARTITION\s+(?:FUNCTION|SCHEME))\s+(?:IF\s+EXISTS\s+)?(?!(?:@|#|##))|TRUNCATE\s+TABLE\s+|EXEC\s+(?:(?:sys\.)?sp_addextendedproperty|(?:\S+\.\S+\.)?sp_addlinkedserver|(?:\S+\.\S+\.)?sp_serveroption)\s+).+$/gmi
