...
export const ALL_DDL_VALID_REGEXP_WITHOUT_CHECK_TEMP_TABLE = new RegExp(ALL_DDL_VALID_REGEXP.source.replaceAll('(?!@|#|##)', '').replaceAll('(?:@|#|##)', ''), 'gmi')
...
export const ALL_DML_VALID_REGEXP_WITHOUT_CHECK_TEMP_TABLE = new RegExp(ALL_DML_VALID_REGEXP.source.replaceAll('(?!@|#|##)', '').replaceAll('(?:@|#|##)', ''), 'gmi')

const MSSQL_SELECT_VALID_REGEXP: RegExp = /^SELECT\s+(?!COUNT\((?:[0-9]+|\*)\)\s+|.+\s+INTO\s+)(?:.+|\*)\s+FROM\s+\S+.+$/gmi
const ORACLE_SELECT_VALID_REGEXP: RegExp = /^SELECT\s+(?!COUNT\((?:[0-9]+|\*)\)\s+|.+\s+INTO\s+)(?:.+|\*)\s+FROM\s+\S+\.+\S+.+$/gmi
export const SELECT_VALID_REGEXP: RegExp = combineRegExps([MSSQL_SELECT_VALID_REGEXP, ORACLE_SELECT_VALID_REGEXP])

const MSSQL_SELECT_COUNT_REGEXP: RegExp = /^(?:Select\s+count\([*|1]\)\s+From\s+\S+).+$/gmi
const ORACLE_SELECT_COUNT_REGEXP: RegExp = /^(?:Select\s+count\([*|1]\)\s+From\s+\S+\.\S+).+$/gmi
export const SELECT_COUNT_REGEXP: RegExp = combineRegExps([MSSQL_SELECT_COUNT_REGEXP, ORACLE_SELECT_COUNT_REGEXP])

export const GRANT_REVOKE_REGEXP: RegExp = /^(?:grant\s+.\S.+to\s+|revoke\s+\S.+from\s+)\S.+$/gmi
export const ANY_COMMAND_REGEXP: RegExp = /^.+$/gmi

export const INSERT_INTO_REGEXP: RegExp = /^(?:insert\s+into\s+(?:@|#|##)\S+\s+Select).+$/gmi

export const ALL_VALID_REGEXP: RegExp = combineRegExps([
